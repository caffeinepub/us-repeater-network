...
export function normalizeHamInfoRepeater(r: HamInfoRepeater): NormalizedRepeater {
  return {
...
export async function fetchHamInfoRepeaters(state?: string): Promise<NormalizedRepeater[]> {
  try {
    ...
    const results = data.results ?? (Array.isArray(data) ? (data as HamInfoRepeater[]) : []);
    ...
    if (state) {
      try {
        ...
        throw new Error('Both HamInfo and RadioReference failed to load repeater data.');
      }
    }
    throw err;
  }
...
async function fetchRadioReferenceRepeaters(state: string): Promise<NormalizedRepeater[]> {
  ...
  const stateInfo = US_STATES.find(
    (s) => s.abbr.toUpperCase() === state.toUpperCase() || s.name.toLowerCase() === state.toLowerCase()
  );
  if (!stateInfo) return [];
