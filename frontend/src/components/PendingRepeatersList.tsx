import { CheckCircle, XCircle, Radio, MapPin, Zap, Clock } from 'lucide-react';
    ...
    <div className="ham-card p-4 space-y-3 animate-fade-in">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap text-sm">
              <span className="font-mono font-semibold text-foreground">{repeater.frequency.toFixed(4)} MHz</span>
              <span className="text-muted-foreground font-mono text-xs">{offsetStr} MHz</span>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5">
          ...
          <span>{repeater.city}, {repeater.state} {repeater.zipCode}</span>
        </div>
    ...
    return (
      <div className="space-y-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="ham-card p-4 space-y-3">
            <div className="flex items-start gap-3">
  ...
  if (repeaters.length === 0) {
    return (
      <div className="ham-card p-8 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">No pending submissions to review.</p>
      </div>
    );
  }
