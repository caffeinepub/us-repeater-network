...
import { AlertCircle, Info, Radio, Send } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addRepeater.mutateAsync({
        ...
        status:
          form.status === "active"
            ? ({ active: null } as unknown as Status)
            : ({ inactive: null } as unknown as Status),
        submittedBy: identity.getPrincipal().toString(),
      });
      toast.success(
        "Repeater submitted for review! An admin will approve it shortly.",
      );
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Submit a GMRS or ham radio repeater not listed in RepeaterBook. All
          submissions are reviewed by an admin before appearing in the
          directory.
        </p>
      </div>
      ...
      <div className="mb-6 flex items-start gap-3 p-4 bg-amber/10 border border-amber/30 rounded-lg">
        ...
        <div className="text-sm text-muted-foreground">
          <strong className="text-amber">Note:</strong> Ham radio repeaters are
          automatically pulled from RepeaterBook.com. This form is primarily for{" "}
          <strong className="text-foreground">GMRS repeaters</strong> and any
          ham repeaters not yet listed in RepeaterBook.
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleChange("coverageDescription", e.target.value)
                }
                className="bg-navy-light border-border text-foreground mt-1"
                rows={2}
              />
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleChange("operationalNotes", e.target.value)
                }
                className="bg-navy-light border-border text-foreground mt-1"
                rows={2}
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("autopatchInfo", e.target.value)
                  }
                  className="bg-navy-light border-border text-foreground mt-1"
                />
              </div>
